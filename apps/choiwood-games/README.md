...
## 最短手順
1. このフォルダ一式を `topicmaker-git/Pages` の中（例 `apps/`）に入れて Commit & Push
2. Settings → Pages を有効化
3. 公開URL `https://topicmaker-git.github.io/Pages/apps/` をスマホの Safari / Chrome で開く
4. ゲームを選ぶ → 録音時にマイク許可
...
1. **置き場所を用意**
   `topicmaker-git` の `Pages` リポジトリ（既存の GitHub Pages 用）を使うのが手軽です。
...
2. **GitHub Desktop でコミット＆プッシュ**
   ...
   - Summary に「add studio app」など入力 → **Commit to main**
   - **Push origin**

3. **Pages を有効化**（初回のみ）
   - GitHub のリポジトリ画面 → **Settings → Pages**
   - Source を `Deploy from a branch`、Branch を `main` / フォルダ `/(root)` → Save
   - 数分待つと公開URLが表示されます

4. **スマホで開く**
   `https://topicmaker-git.github.io/Pages/apps/studio.html` のような URL を
